...
- Storage Elevated: utilization ≥80%; Critical: utilization ≥90% or free bytes <10 GiB; Full: utilization ≥98% or zero free bytes. The most severe known condition wins. Zero is the conservative no-usable-space cutoff; no speculative nonzero reserve is inferred. Exactly 10 GiB does not trigger the low-space condition.
- Uptime warning: ≥30 days (2,592,000 seconds).

Flags remain separate. Either known disk violation can warn; if neither violates and a required disk input is missing, the disk check is unknown. Healthy/current requires all four checks known and non-warning. A partial current sample with no known warnings is unknown/current. Stale data never yields Healthy/current or generates current alerts. Stored values remain visible as last-reported measurements. Online/offline continues using existing heartbeat semantics and is not inferred from health.
...
Presentation alerts aggregate to Healthy, Attention (CPU/memory/uptime high or storage elevated), Critical (storage critical/full), Stale or Unknown. Online/offline connectivity remains independently displayed; Offline is never conflated with stale hardware telemetry. Shared alert reasons are reused in the endpoint summary, health panel, hardware cards and compact fleet summary. No collection, scheduling or persistence changes accompany these presentation thresholds.
